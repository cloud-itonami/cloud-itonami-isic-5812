(ns dirmailops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (DirMailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the DirMailGovernor (:govern)
  ...
  One graph run = one publishing-coordination request (intake -> advise
  ...
  human operator/editorial coordinator. The approver resumes with
...
(defn build
  ...
  `dirmailops.store/Store`).
  opts:
    :advisor      -- a `dirmailops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; DirMailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; DirMailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
